...
with open(Path(__file__).parent / "assets" / "vghoi_instance_meta.json") as f:
    meta = json.load(f)
...
def doc_to_visual(doc: dict[str, Any]) -> list[Image.Image]:
    """Convert an image from a VG-HOI document to RGB format.

    Args:
    ----
        doc (dict): A dictionary containing VG-HOI document data with image information.

    """
    filename = os.path.join(os.environ["VGHOI_IMAGES_DIR"], doc["image_filename"])
...
def doc_to_visual_reverse_blur(doc: dict[str, Any]) -> list[Image.Image]:
    """Convert an image from a VG-HOI document to RGB format with a reverse blur effect.
...
def doc_to_text(doc: dict[str, Any], model_specific_kwargs: dict[str, str] | None = None) -> str:
    """Convert a VG-HOI document to a question string."""
...
def doc_to_target(doc: dict[str, Any], model_specific_kwargs: dict[str, str] | None = None) -> str:
    """Convert a VG-HOI document to an empty target string."""
    return ""
...
def create_results_file(items: list[dict], args: dict) -> None:
    """Create a JSON file containing VG-HOI triplets for evaluation.
